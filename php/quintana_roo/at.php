...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!$imgC) {
        ...
        $upload_dir = '../upload/quintana_roo/';
        $foto_path = $upload_dir . basename($foto);

        // Preparar la consulta SQL
        $sql = "UPDATE quintana_roo SET nombre=?, descripcion=?, ubicacion=?, costo_viaje=?, foto=? WHERE id=?";
        ...
        // Preparar la consulta SQL
        $sql = "UPDATE quintana_roo SET nombre=?, descripcion=?, ubicacion=?, costo_viaje=? WHERE id=?";
        $declaracion = $conexion->prepare($sql);
